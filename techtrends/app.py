...
def post(post_id):
    ...
    if post is None:
      app.logger.info('Article with id: ' + str(post_id) + ' does not exist!')
...
def metrics():
    ...
    postsCount = posts.fetchone()[0]

    response = app.response_class(
            response=json.dumps({"db_connection_count":str(total_connections),"post_count":str(postsCount)}),
            status=200,
            mimetype='application/json'
    )
